...
foreach($_POST['sites'] as $group_name_key=>$sites_arr){
	...
	while($rs=$DB->fetch_assoc($query)){
		...
		//水样类型
		$result[$rs['c_date']][$rs['id']]['water_type']=$rs['water_type'];
		//表头数据
		$result[$rs['c_date']][$rs['id']][bt]['sgnq_code']=$rs['sgnq_code'];//水功能区序号
		$result[$rs['c_date']][$rs['id']][bt]['sgnq']=$rs['sgnq'];//水功能区名称
		$result[$rs['c_date']][$rs['id']][bt]['site_name']=$rs['site_name'];//控制断面
		$result[$rs['c_date']][$rs['id']][bt]['xz_area']=$rs['xz_area'];//行政区
		$result[$rs['c_date']][$rs['id']][bt]['cy_date']=$rs['c_date'];//采样日期
		$result[$rs['c_date']][$rs['id']][bt]['cy_time']=substr($rs['cy_time'],0,5);//采样时间
		$result[$rs['c_date']][$rs['id']][bt]['water_height']=$rs['water_height'];//水位
		$result[$rs['c_date']][$rs['id']][bt]['liu_l']=$rs['liu_l'];//流量
		$result[$rs['c_date']][$rs['id']][bt]['qi_wen']=$rs['qi_wen'];//流量/蓄水量
		//化验项目数据
		...
		$result[$rs['c_date']][$rs['id']]['vid'][$rs['vid']]['vd0']=$vd0;
...
foreach ($result as $key_sid => $value_site_arr) {
	...
	if($fenye	== 'yes'){
		$bt_water_type[$yeshu]	.= "<td  style='max-width:130px;'>标准</td>";
		$bt_site[$yeshu]		.= "<td>$jcbz_name</td>";
		$yeshu++;
		//$site_num	= 1;
		$bt_water_type[$yeshu]	.= "<td rowspan='2' style='width:60px'>序号</td><td rowspan='2' nowrap>项目</td><td rowspan='2' nowrap style='width:60px'>单位</td>";
		$fenye	= '';
	...
	if(!in_array($value_site_arr['bt']['cy_date'],$old_water_type) || (@in_array($value_site_arr['bt']['cy_date'],$old_water_type) && !in_array($yeshu,$old_water_type_ye))){
		$old_water_type[]	= $value_site_arr['bt']['cy_date'];
		$old_water_type_ye[]= $yeshu;
		if($last_num>=$water_type_num[$value_site_arr['bt']['cy_date']]){
			$last_num	= $last_num-$water_type_num[$value_site_arr['bt']['cy_date']];
			$bt_water_type[$yeshu]	.= "<td colspan='{$water_type_num[$value_site_arr['bt']['cy_date']]}'>{$value_site_arr['bt']['cy_date']}</td>";
		}else{
			$water_type_num[$value_site_arr['bt']['cy_date']]	= $water_type_num[$value_site_arr['bt']['cy_date']]-$last_num;
			$bt_water_type[$yeshu]	.= "<td colspan='{$last_num}'>{$value_site_arr['bt']['cy_date']}</td>";
		}
	}
	$bt_site[$yeshu]	.= "<td style='min-width:45px;'>{$value_site_arr['bt']['site_name']}</td>";
	//项目信息
	$xuhao	= $vid_num	= 0;
	$tr_fenye	= 1;
	foreach ($vid_arr as $key_vid => $value) {
		...
		if(count($result)==$site_num || $fenye=="yes" ){
			if($tr_fenye_zt=='yes'){
				$content[$yeshu][$tr_fenye][$key_vid]	.= "<td  style='max-width:130px;'>{$jcxz_arr[$max_water_type][$key_vid]}</td>";
				$tr_fenye_zt	= '';
			}else{
				$content[$yeshu][$tr_fenye][$key_vid]	.= "<td  style='max-width:130px;'>{$jcxz_arr[$max_water_type][$key_vid]}</td>";
			}
