...
###############站点选择区域###########
$alone_sites_str	= '';
if(!empty($info['alone_sites'])){
	$i	= 0;
	foreach ($info['alone_sites'] as $group_name=>$site_name_arr) {
		$i++;
		$alone_sites_str	.= "<div style='clear:left;text-align:left;'><label><input name=\"alone_group_name[]\" type=\"checkbox\" value='{$group_name}' site_group='yes' bz='{$i}' checked /><span class='pc_css'>{$group_name}</span></label></div><div>";
		foreach($site_name_arr as $site_name){
			//$alone_sites_str.= "<span class='s_float'><label><input type='checkbox' name='alone_sites[]' value='{$site_name}' checked />".$site_name."</label></span>";
			$alone_sites_str	.= "<span class=\"s_float\"><label><input  name=\"alone_sites[{$group_name}][]\" type=\"checkbox\" value=".$site_name." site='yes' bz='{$i}' checked />".$site_name."</label></span>";
		}
	}
}
###############项目选择区域####################
...
if(!empty($info['alone_vid'])){
	$value_num	= count($info['alone_vid']);
