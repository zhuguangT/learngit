...
if(!is_dir("/tmp/text"))mkdir("/tmp/text",0777);
if(!file_exists("/tmp/text/".$pname.".text"))exec("pdftotext $lujing /tmp/text/{$pname}.text");
$lujing2 = "/tmp/text/".$pname.".text";
		...
		foreach($temp_arr as $key=>$value){
			...
			if($get_xmzt){
				...
				if($xmArr[$value]){
					...
					if(stristr($last_sj[3],'E-')){
						$value=NumToStr($last_sj[3]);
					}
					elseif(stristr($last_sj[3],'-')){
						$value=0;
					}else{
						$value=isset($last_sj[3]) ? $last_sj[3] : 0;
					}
					$last_sj = array();
