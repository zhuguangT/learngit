...
this.each(function(i) { var $tbl = $(this);
...
$clonedTable.attr("id", "fixedtableheader" + i).css({ "background-color":"#fff", "position": "fixed", "top": "0", "left": $tbl.offset().left }).append($tblhfixed.clone()).width(tblwidth).hide().appendTo($("body"));
if (settings.highlightrow) $("tr:gt(" + (settings.headerrowsize - 1) + ")", $tbl).hover(function() { $(this).addClass(settings.highlightclass);
}, function() { $(this).removeClass(settings.highlightclass);
});
...
$(window).resize(function() { if ($clonedTable.outerWidth() != $tbl.outerWidth()) { $tblhfixed.find(headerelement).each(function(index) { var w = $(this).width();
$(this).css("width", w);
...
function GetTblWidth($tbl) { var tblwidth = $tbl.outerWidth();
return tblwidth;
} };
