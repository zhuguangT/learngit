...
Calendar.prototype.create=function(_par){
  var parent_ele=null;
  if(!_par){
    parent_ele=document.getElementsByTagName("body")[0];
    this.isPopup=true;
  }else {
    parent_ele=_par;
    this.isPopup=false;
  ...
  parent_ele.appendChild(this.element);
};
...
opentime.cal = null;
/**
 * 单击一个按钮打开日历控件,并与与按钮对应的文本框关联
 *
 * @param string id 按钮的id
 * @param array date_range 允许选择的日期列表. 要允许所有,传递"all"
 * @param dateFormat string
 * @return void
 */
function opentime(id,date_range,dateFormat){
    var input = document.getElementById(id);
    var cal = opentime.cal;
    if(!cal){
        ...
        var format = dateFormat===undefined?'%Y-%m-%d':dateFormat;
        cal.weekNumbers = false;
        cal.showsOtherMonths = true;
        cal.setRange(2002, 2015);
        cal.setDateFormat(format);
        cal.create();
        opentime.cal = cal;
    }

    var disallowDate = _disallowDate(date_range,format);
    cal.setDisabledHandler(disallowDate);
    cal.onSelected = function(_input,func){
        return function(calendar,date_str){
            var date = calendar.date;
            if(!func(date)){
                _input.value = date_str;
    ...
    cal.parseDate(input.value||date_range[0]||'');
    cal.showAtElement(input,'Br');
}
...
function _disallowDate(allow_dates,dateFormat){
    return function(date){
        if(allow_dates=='all') return false;
        if(allow_dates.indexOf(date.print(dateFormat))>-1||isNaN(date.getFullYear()))
            return false;
        return true;
    }
}
