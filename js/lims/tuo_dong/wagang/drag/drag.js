...
var mix=QW.ObjectH.mix,
	//lazyApply=QW.FunctionH.lazyApply,//dev 版
	/*
...
(function(){
	...
	var mouseM=function(e)//mouseMove
	{
		...
		DragManager.mouseDownTarget.style.backgroundColor="#C0C0E0";
	};
...
(function(){
	...
	var $F=function(s){return parseFloat(s)||0;};
	SimpleDrag.prototype={
		...
		getProxy: function(){
			...
			return function(){
				...
				if(!el){
					el=createElement('div',{className:'proxy-dd'});
		...
		dragstart: function(e){
			...
			if(me.withProxy) {
				...
				lazyApply(
					function(){proxy.style.display='block';},
					null,
					[],
					20,
					function(){
	...
	RectSelector.prototype={
		...
		getProxy: function(){
			...
			return function(){
				var el=this.oProxy||proxy;
				if(!el){
					el=createElement('div',{className:'proxy-rectselector'});
					document.body.appendChild(el);
					el.style.display='none';
				}
				return this.oProxy=el;
...
(function(){
	...
	LayoutDrag.prototype={
		withProxy:true,
		isInline:false, //是否是inline的Layout.
		dragstart:function(e){
		...
		adjustLayout: function(custEvent){
			...
			if(custEvent.type=='dragend'){
				if(me.needAnim && QW.ElAnim){
					...
					var elAnim=new QW.ElAnim(
						me.oProxy,
						{width:{to:rect.width}, height:{to:rect.height}, left:{to:rect.left}, top:{to:rect.top}},
						300
					);
					elAnim.on('suspend',function(){me.oProxy.style.display='none';});
					elAnim.play();
