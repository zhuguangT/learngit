 ...
 * @last-modified : 2009/9/8 by JK
*/
/**
 * @param {Number} p:per当前播放进度
 */
 (function() {
	var Easing  = {
		easeNone: function(p) {
		...
		elasticBoth: function(p) {
			if(p==0)return 0;
			if ((p/=0.5)==2)return 1;
			var x=.3*1.5,y=1,z=x/4;
			if(p<1)return -.5*(Math.pow(2,10*(p-=1))*Math.sin((p-z)*(2*Math.PI)/x));
			return Math.pow(2,-10*(p-=1))*Math.sin((p-z)*(2*Math.PI)/x )*.5+1;
		},
		backIn: function(p) {
			var s=1.70158;
			return p*p*((s+1)*p-s);
		...
		bounceBoth: function(p) {
			if(p<0.5)return Easing.bounceIn(p*2)*.5;
			return Easing.bounceOut(p*2-1)*.5 + 0.5;
		}
	};
