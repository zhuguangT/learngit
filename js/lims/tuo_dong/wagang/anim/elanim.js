 ...
 * @author:{@link mailto:quguangyu@baidu.com Jerry Qu}
 * @last-modified : 2009/9/8
 * @part-modified : 2009/11/02 by JK
 */
...
(function() {
	...
	*/
	var ElAnim = function(el, attrs, dur, easing) {
			//if(!NodeH.isElement(g(el))) throw new Error(['Animation','Initialize Error','Element Not Found!']);
			this.el = g(el);
			this.attrs = attrs;
			dur = dur || ElAnim.DefaultDur;
			this.easing = typeof easing == "function" ? easing : ElAnim.DefaultEasing;
			var anim=new Anim(function(per){this.onTween(per);}, dur);
			mixMentor(this,anim);//******
	...
	mix(ElAnim.prototype,{
		...
		getUnits : function(attr) {
			var unit='',tatterns=ElAnim.UNIT_PATTERNS;
	...
	*/
	...
	QW.provide({ElAnim:ElAnim, ScrollAnim:ScrollAnim, ColorAnim:ColorAnim});
})();